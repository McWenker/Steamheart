...
public class TestItems : MonoBehaviour 
{
	...
	GameObject CreateActor (string title)
	{
		...
		s[StatTypes.MEL] = UnityEngine.Random.Range(30, 50);
	...
	void CreateItems ()
	{
		...
		inventory.Add( CreateEquippableItem("Sword", StatTypes.MEL, 10, EquipSlots.Primary) );
		inventory.Add( CreateEquippableItem("Broad Sword", StatTypes.MEL, 15, (EquipSlots.Primary | EquipSlots.Secondary)) );
	...
	void Attack (GameObject attacker, GameObject defender)
	{
		...
		int damage = Mathf.FloorToInt((s1[StatTypes.MEL] * 4 - s2[StatTypes.DEF] * 2) * UnityEngine.Random.Range(0.9f, 1.1f));
	...
	void EquipItem (GameObject item)
	{
		...
		equipment.Equip (toEquip, toEquip.defaultSlots, combatants[0].transform);
	}
	...
	void LogToConsole (GameObject actor)
	{
		...
		string message = string.Format("Name:{0} HP:{1}/{2} ATK:{3} DEF:{4}", actor.name, s[StatTypes.HP], s[StatTypes.MHP], s[StatTypes.MEL], s[StatTypes.DEF]);
		Debug.Log( message );
	}
	#endregion
}